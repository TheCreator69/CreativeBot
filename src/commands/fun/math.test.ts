import {isNumberMultipleOf, generateMathQuestion, calculateTimeForAnswering, MathQuestion} from "./math";

var mathQuestionToTest: MathQuestion;

beforeAll(() => {
    mathQuestionToTest = generateMathQuestion();
});
...
test("Checks if the numbers '10' and '5' return 'true'", () => {
    expect(isNumberMultipleOf(10, 5)).toBeTruthy();
});

test("Checks if the numbers '5' and '2' return 'false'", () => {
    expect(isNumberMultipleOf(5, 2)).toBeFalsy();
});

test("Checks if the numbers '19' and '1' return 'true'", () => {
    expect(isNumberMultipleOf(19, 1)).toBeTruthy();
});
