...
function createCorrectHelpEmbed(message: Message, args: string[]): MessageEmbed {
    if(!args.length) {
        ...
        if(doesCommandExist(args)) {
            return createHelpEmbed("#499fff", ":ledger: Help for: " + getCommandInstance(args).info.name, createCommandInfoString(args), "Arguments wrapped with \"<>\" are required, others wrapped with \"[]\" are optional.");
        }
        else {
...
function createHelpEmbed(color: string, title: string, description: string, footer: string): MessageEmbed {
